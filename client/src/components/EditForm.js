...
function CreateEventForm({ onClose, onEventChange, eventId }) {
  ...
  const fetchEventDetail=useCallback(async()=>{
    const response = await fetch(URL + "/api/events/"+eventId);
      ...
      setDateTime(new Date(data.data.date_time).toISOString().split("T")[0]);
      setLocation(data.data.location);
      setMaxCapacity(data.data.max_capacity);
